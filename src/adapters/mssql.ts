  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      throw new DatabaseAdapterError(
        `Failed to connect to SQL Server: ${toError(error).message}`,
        DatabaseType.MSSQL,
        'connect',
        toError(error)
      );
  ...
  async query<T = Record<string, unknown>>(
    ...
    try {
      ...
      if (params && params.length > 0) {
        // Replace all $N placeholders in a single pass to avoid $1 matching
        // inside $10, $11, etc. when doing sequential replacement
        convertedSql = convertedSql.replace(
          /\$(\d+)/g,
          (_match, num) => `@p${parseInt(num, 10) - 1}`
        );
      ...
      throw new DatabaseAdapterError(
        `Query execution failed: ${toError(error).message}`,
        DatabaseType.MSSQL,
        'query',
        toError(error)
      );
