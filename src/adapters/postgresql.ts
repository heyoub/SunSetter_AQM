...
export class PostgreSQLAdapter extends BaseAdapter {
  ...
  private buildSSLConfig(
    ...
    if (sslConfig.ca) {
      try {
        ...
        throw new DatabaseAdapterError(
          `Failed to load CA certificate from ${sslConfig.ca}: ${toError(error).message}`,
    ...
    if (sslConfig.cert) {
      try {
        ...
        throw new DatabaseAdapterError(
          `Failed to load client certificate from ${sslConfig.cert}: ${toError(error).message}`,
          DatabaseType.POSTGRESQL,
          'ssl-config',
          toError(error)
        );
    ...
    if (sslConfig.key) {
      try {
        ...
        throw new DatabaseAdapterError(
          `Failed to load client key from ${sslConfig.key}: ${toError(error).message}`,
          DatabaseType.POSTGRESQL,
          'ssl-config',
          toError(error)
        );
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      throw new DatabaseAdapterError(
        `Failed to connect to PostgreSQL: ${toError(error).message}`,
        DatabaseType.POSTGRESQL,
        'connect',
        toError(error)
      );
  ...
  async query<T = Record<string, unknown>>(
    ...
    try {
      ...
      throw new DatabaseAdapterError(
        `Query failed: ${toError(error).message}`,
        DatabaseType.POSTGRESQL,
        'query',
        toError(error)
      );
  ...
  async queryInTransaction<T = Record<string, unknown>>(
    ...
    try {
      ...
      throw new DatabaseAdapterError(
        `Transaction failed: ${toError(error).message}`,
        DatabaseType.POSTGRESQL,
        'transaction',
        toError(error)
      );
