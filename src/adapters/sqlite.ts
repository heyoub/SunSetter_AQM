...
import { ConnectionError } from '../cli/errors/index.js';
...
export class SQLiteAdapter extends BaseAdapter {
  ...
  async *streamRows(
    ...
    // When using rowid, we must select it explicitly since SELECT * doesn't include it
    const useRowid = !orderBy;
    ...
    while (hasMore) {
      ...
      // When ordering by rowid, explicitly include it in the SELECT so
      // we can read the cursor value from the result rows
      ...
      lastCursor = lastRow[orderBy || 'rowid'] as string | number;
