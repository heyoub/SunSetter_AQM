...
export class SummaryReportGenerator {
  ...
  constructor(toolVersion: string = '1.6.0') {
  ...
  createExtendedSummary(
    ...
    return {
      ...
      performance: {
        ...
        avgRowsPerSecond:
          durationSec > 0 ? Math.round(totalRows / durationSec) : 0,
  ...
  private generateTextReport(summary: ExtendedMigrationSummary): string {
    ...
    lines.push('─'.repeat(60));
    lines.push(
      `  Throughput:    ${performance.avgRowsPerSecond.toLocaleString()} rows/sec`
    );
  ...
  private generateMarkdownReport(summary: ExtendedMigrationSummary): string {
    ...
    lines.push('');
    lines.push(
      `**Status:** ${report.status === 'completed' ? '✅' : report.status === 'partial' ? '⚠️' : '❌'} ${report.status}`
    );
    lines.push(`**Generated:** ${environment.timestamp}`);
    lines.push('');
...
<body>
    ...
    <table>
      ...
      ${report.tables
        .map((t) => {
          const pct =
            t.totalRows > 0
              ? Math.round((t.migratedRows / t.totalRows) * 100)
              : 100;
  ...
  <div class="card">
    ...
    ${report.errors
      .slice(0, 10)
      .map(
        (e) => `<p class="error"><strong>[${e.code}]</strong> ${e.message}</p>`
      )
      .join('')}
...
export function printSummaryToConsole(summary: ExtendedMigrationSummary): void {
  ...
  const statusIcon =
    report.status === 'completed'
      ...
      : report.status === 'partial'
        ...
        : '✖';
  console.log(
    `  Status: ${statusColor.bold(`${statusIcon} ${report.status.toUpperCase()}`)}`
  );
  console.log('');
  ...
  console.log(
    `  ${chalk.cyan('Total Rows:')}    ${report.totalRows.toLocaleString()}`
  );
  console.log(
    `  ${chalk.cyan('Migrated:')}      ${chalk.green(report.migratedRows.toLocaleString())}`
  );
  if (report.failedRows > 0) {
    console.log(
      `  ${chalk.cyan('Failed:')}        ${chalk.red(report.failedRows.toLocaleString())}`
    );
  }
  console.log(
    `  ${chalk.cyan('Duration:')}      ${Math.round(report.duration / 1000)}s`
  );
  console.log(
    `  ${chalk.cyan('Throughput:')}    ${performance.avgRowsPerSecond.toLocaleString()} rows/sec`
  );
  console.log(chalk.gray('  ─'.repeat(29)));
  console.log('');
