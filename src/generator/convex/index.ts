...
export class ConvexFunctionGenerator {
  ...
  generate(tables: TableInfo[]): ConvexGeneratedOutput {
    const output: ConvexGeneratedOutput = {
      ...
      stats: {
        ...
        totalActions: 0,
        totalHttpActions: 0,
        totalSearchIndexes: this.schemaGenerator.getSearchIndexCount(tables),
        totalVectorIndexes: this.schemaGenerator.getVectorIndexCount(tables),
      },
    };

    // 1. Generate main schema.ts (includes search and vector indexes if enabled)
    output.schema = this.schemaGenerator.generate(tables);
    ...
    output.indexFile = this.generateIndexFile(tables);
  ...
  private generateIndexFile(tables: TableInfo[]): string {
    ...
    // Per-table exports
    for (const table of tables) {
      const pascalName = this.toPascalCase(table.tableName);
      const camelName = this.toCamelCase(table.tableName);
