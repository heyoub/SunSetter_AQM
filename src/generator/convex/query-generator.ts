...
export interface QueryGeneratorOptions {
  ...
  /** Emit search helpers only when matching search indexes exist in schema */
  generateSearchQueries?: boolean;
}

const DEFAULT_OPTIONS: QueryGeneratorOptions = {
  convexApiVersion: '1.30',
  generateSearchQueries: false,
};

/**
 * Generates Convex query functions
 */
export class QueryGenerator extends BaseConvexGenerator<QueryGeneratorOptions> {
  ...
  generate(table: TableInfo): QueryGeneratorResult {
    ...
    if (this.options.generateSearchQueries && textFields.length > 0) {
 ...
 *   const ${toCamelCase(tableName)} = useQuery(api.${tableName}.get, { id });
 *   return <div>{${toCamelCase(tableName)}?.name}</div>;
 * }
 * \`\`\`
 *
...
export const ${methodName} = query({
  ...
  handler: async (ctx, args) => {
    ...
    for (const idx of uniqueFields) {
      ...
      const methodName = `getBy${toPascalCase(idx.columnName)}`;
