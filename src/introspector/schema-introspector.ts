...
import { normalizeTableInfo } from './normalize-schema.js';
...
export class SchemaIntrospector {
  ...
  async introspectSchema(schemaName: string = 'public'): Promise<SchemaInfo> {
    const tables = (await this.getTables(schemaName)).map(normalizeTableInfo);
    const views = (await this.getViews(schemaName)).map(normalizeTableInfo);
  ...
  async getAllSchemas(): Promise<string[]> {
    ...
    const result = await this.adapter.query<{ schema_name: string }>(query);
  ...
  getConvexTableName(
    ...
    // Only prefix when the schema is not the database's default
    const defaultSchema = this.adapter.getDefaultSchema();
    if (prefixTableNames && schemaName !== defaultSchema) {
  ...
  parseConvexTableName(
    ...
    return {
      schemaName: this.adapter.getDefaultSchema(),
  ...
  private async getViews(schemaName: string): Promise<TableInfo[]> {
    ...
    const views = await this.adapter.query<{
      table_name: string;
      table_schema: string;
      table_type: 'VIEW';
      table_comment: string | null;
  ...
  private async getPrimaryKeys(
    ...
    const result = await this.adapter.query<{ column_name: string }>(query, [
      schemaName,
      tableName,
    ]);
  ...
  private async getIndexes(
    ...
    const indexes = await this.adapter.query<{
      index_name: string;
      is_unique: boolean;
      index_method: string;
