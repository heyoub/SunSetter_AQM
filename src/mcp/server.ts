...
import { ConfigurationError } from '../cli/errors/index.js';
        ...
        inputSchema: {
          ...
          properties: {
            schema: {
              type: 'string',
              description:
                'Schema name to introspect (default: auto-detected — "public" for PostgreSQL, database name for MySQL, "dbo" for SQL Server, "main" for SQLite)',
            },
            includeRowCounts: {
              type: 'boolean',
              description:
    ...
    switch (name) {
      ...
      case 'get_table_info': {
        ...
        if (includeSampleData && rowCount > 0) {
          const stream = adapter.streamRows(defaultSchema, tableName, {
