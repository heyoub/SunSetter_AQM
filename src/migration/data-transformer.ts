...
import { toCamelCase, escapeFieldName } from '../utils/naming.js';
import type {
  PostgresRow,
  ConvexDocument,
  IIdMapper,
...
import { TransformError } from '../cli/errors/index.js';
...
export class DataTransformer {
  ...
  private transformByType(value: unknown, column: ColumnInfo): unknown {
    ...
    switch (baseType) {
      ...
      // Vector / embeddings
      case 'vector':
        return this.transformVector(value);
  ...
  /**
   * Transform pgvector values into numeric arrays for Convex vector indexes.
   */
  private transformVector(value: unknown): number[] {
    if (Array.isArray(value)) {
      return value.map((item) => this.toFiniteNumber(item, 'vector element'));
    }

    if (typeof value !== 'string') {
      throw new TransformError(
        `Expected vector to be a string or array, got ${typeof value}`
      );
    }
    ...
    try {
      ...
      throw new TransformError(
        `Invalid vector format: ${(error as Error).message}`
      );
    }

    if (!Array.isArray(parsed)) {
      throw new TransformError('Invalid vector format: expected an array');
    }

    return parsed.map((item) => this.toFiniteNumber(item, 'vector element'));
  }

  private toFiniteNumber(value: unknown, label: string): number {
    const parsed = typeof value === 'number' ? value : Number(value);
    if (!Number.isFinite(parsed)) {
      throw new TransformError(`Invalid ${label}: ${String(value)}`);
    }
    return parsed;
  }
  ...
  private getFieldName(
    ...
    // Convert to camelCase and escape reserved words if enabled
    if (this.config.convertFieldNames) {
      return escapeFieldName(toCamelCase(column.columnName));
    }
