...
export class ParallelMigrator {
  ...
  private sharedTokenBucket: TokenBucket;
  ...
  constructor(
    ...
    // Create a single shared token bucket so all parallel table migrators
    // respect the same rate limit instead of each having an independent one
    this.sharedTokenBucket = {
  ...
  private async migrateTable(table: TableInfo): Promise<TableMigrationResult> {
    const migrator = new TableMigrator(
      ...
      this.stateManager,
      {
        ...
        sharedTokenBucket: this.sharedTokenBucket,
      }
    );
