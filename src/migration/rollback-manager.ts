...
export class RollbackManager {
  ...
  async createTableSnapshot(
    ...
    if (!this.currentState) {
      throw new DataMigrationError('No active rollback state');
    }
      ...
      if (options.deleteFromConvex !== false) {
        try {
          ...
          const migrationError: MigrationError = {
            code: 'CONVEX_ERROR',
            message: `Failed to rollback table ${tableName}: ${toError(error).message}`,
            table: tableName,
  ...
  async load(migrationId: string): Promise<RollbackState | null> {
    ...
    try {
      ...
      if ((toError(error) as NodeJS.ErrnoException).code === 'ENOENT') {
        return null;
      }
      throw error;
    }
