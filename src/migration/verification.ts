...
export class MigrationVerifier {
  ...
  private async verifySamples(tableName: string): Promise<SampleCheckResult[]> {
    ...
    for (const row of samples) {
      ...
      for (const [field, sourceValue] of Object.entries(row)) {
        if (this.options.skipFields.includes(field)) continue;

        // Transform the source field name the same way data-transformer does:
        // snake_case → camelCase, then escape reserved words
        const convexFieldName = escapeFieldName(toCamelCase(field));
        if (this.options.skipFields.includes(convexFieldName)) continue;

        fieldsChecked++;
        const convexValue = convexDoc[convexFieldName];
        const comparison = this.compareValues(sourceValue, convexValue);
        ...
        if (comparison.match) {
          ...
          mismatches.push({
            ...
            sourceValue,
            convexValue,
            reason: comparison.reason ?? 'Values differ',
          });
        }
      }
