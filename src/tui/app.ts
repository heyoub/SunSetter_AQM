...
export class TUIApp {
  ...
  private async runMigration(): Promise<void> {
    ...
    const targetSchema = schemas.includes(defaultSchema)
      ? defaultSchema
      : schemas[0] || defaultSchema;
  ...
  private async runGenerate(): Promise<void> {
    ...
    try {
      ...
      // Parse and connect using adapter
      const config = parseConnectionString(connectionString);
      const genAdapter = createAdapter(config);

      try {
        await genAdapter.connect();
      } catch {
  ...
  private async runIntrospect(): Promise<void> {
    ...
    try {
      ...
      // Parse and connect
      const config = parseConnectionString(connectionString);
      const introAdapter = createAdapter(config);

      try {
        await introAdapter.connect();
      } catch {
        console.log(chalk.red('✗ Failed to connect to database'));
        console.log('\nPress any key to return...');
        await this.waitForKey();
