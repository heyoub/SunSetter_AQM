...
function createMockAdapter(config: {
  ...
  return {
    ...
    getTableRowCount: jest.fn().mockImplementation((schema, table) => {
      return Promise.resolve(rowCounts[table] || 0);
    }),
    streamRows: jest.fn().mockImplementation(function* (schema, table) {
...
function createMockConvexClient(config: {
  ...
  const { documentCounts = {}, documents = {} } = config;
